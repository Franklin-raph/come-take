...
const TopCategories = ({categoryArray}) => {
  ...
  return (
    <div className='grid grid-cols-3 lg:grid-cols-6 items-center justify-between gap-3'>
        {categoryArray && categoryArray.map(item => (
            <div className='relative h-[40vh] category-section rounded-[6px] overflow-hidden' onClick={() => navigate(`/search-product-category?product_category=${item.text}`)}>
